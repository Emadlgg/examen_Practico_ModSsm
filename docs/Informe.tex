...
\caption{Ratio de preservación estructural y fracción de viviendas dañadas habitables por zona.}
\label{tab:habitabilidad}
\end{table}
...
\begin{enumerate}
    ...
    \item \textbf{La paradoja de capacidad ociosa en Z4 (solo con datos propios):} Con base únicamente en el daño estructural reportado en nuestro Excel, la zona Z4 (Este comercial) es la \textbf{única que nunca se satura}. Inicia con un 21.7\% de ocupación y finaliza en 38.9\% (un promedio de 2,068 personas albergadas para una capacidad de 5,300 plazas). Esto significa que durante toda la emergencia existen más de 3,200 camas desocupadas en Z4, mientras en Z1 y Z5 decenas de miles de personas duermen a la intemperie. \textit{Adelanto: esta conclusión no se sostiene al incorporar el output real del Grupo 6 --- ver Sección 4.}
...
\subsection{Estimación inicial para los Grupos 2 y 3 (pre-intercambio)}
\begin{itemize}
...
\end{itemize}
Esta es la estimación con la que trabajamos internamente antes del intercambio; \textbf{no} es la que finalmente se entregó a los Grupos 2 y 3, porque el intercambio se trabajó de forma secuencial (ver Sección 4.2).
...
Durante el intercambio presencial, el Grupo 6 nos entregó formalmente el archivo \texttt{output\_a\_flujo\_desplazados.csv}: la proyección real de evacuación por zona y bloque, calculada con su modelo de comportamiento poblacional (ABM) y separada en dos destinos por persona: \textit{refugio oficial} y \textit{otra zona / redes informales}. Sustituimos únicamente el input $N_z(t)$ de nuestra función \texttt{simular()} por la columna \texttt{nuevos\_hacia\_refugio\_oficial} de ese archivo —la única que compite por la capacidad de los albergues formales que modelamos— sin modificar la función, las capacidades operativas ni ninguna otra parte del modelo.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{grafica6_comparativa_intercambio.png}
    \caption{Comparación de la llegada de desplazados y acumulación de personas sin albergue, antes y después de incorporar la proyección real del Grupo 6 (suma de las 5 zonas).}
    \label{fig:intercambio}
\end{figure}

\textbf{¿En cuántas horas difiere el momento de colapso de los albergues entre ambas estimaciones?}
\begin{itemize}
    \item En \textbf{Z1, Z2, Z3 y Z5} el momento de saturación (>80\%) \textbf{no cambia: 0 horas de diferencia}. Ambas estimaciones colapsan desde el Bloque 0 (0--6h) en esas cuatro zonas.
    \item En \textbf{Z4} el cambio es \textbf{cualitativo, no temporal}: en el escenario base Z4 \textbf{nunca} cruzaba el 80\% (máximo 38.9\%); con el flujo real del Grupo 6, Z4 satura casi de inmediato (96.5\% en el Bloque 0, 100\% desde el Bloque 1). No es que el colapso se adelante un número de horas: es que una zona que antes no colapsaba, ahora sí lo hace desde el primer bloque.
    \item La diferencia real entre ambas estimaciones no está, entonces, en el \textit{momento} de colapso, sino en la \textbf{magnitud}: el total de personas sin alojamiento al final de las 72h pasa de \textbf{42,200} (base) a \textbf{118,611} (Grupo 6) —casi 2.8 veces más— y la demanda médica total pasa de 9,137 a 23,474 personas (2.6 veces más).
\end{itemize}

\textbf{¿Qué supuesto de nuestro modelo inicial era incorrecto y cómo se corrigió?}
El supuesto que resultó incorrecto no fue uno de \textit{retención o retraso en la llegada} —hipótesis razonable que habíamos planteado antes de recibir el output formal— sino uno más fundamental: el \textbf{volumen total de demanda de refugio formal}. Nuestro modelo base estimaba esa demanda a partir de la variable ``necesitan albergue urgente'' del propio Excel de daño estructural, que acumula 50,760 personas en toda la ciudad al final de las 72h. El output real del Grupo 6 —que ya filtra a quienes efectivamente buscan un albergue formal, en lugar de quedarse con familiares o desplazarse a otra zona— proyecta un total de \textbf{130,411 personas}: 2.6 veces más que la estimación basada solo en daño estructural.

Esto invalida el hallazgo más visible de nuestro análisis pre-intercambio (Sección 3, ``paradoja de capacidad ociosa en Z4''): Z4 parecía tener margen porque su daño estructural reportado es bajo, pero el modelo de comportamiento del Grupo 6 muestra que una fracción importante de la población de la ciudad —no solo de Z4— igual intenta acceder a refugio formal en cantidades que la capacidad de Z4 tampoco alcanza a absorber. \textbf{Corrección aplicada:} reemplazamos únicamente el input $N_z(t)$ de \texttt{simular()} por el dato real del Grupo 6, conservando intacta la función y las capacidades operativas, tal como estaba previsto desde el diseño del modelo (Sección 2).

\subsection{Entrega final actualizada a los Grupos 2 y 3}
El intercambio de este examen se trabajó de forma \textbf{secuencial}: primero se recibió el output formal del Grupo 6 y, ya con ese dato incorporado, se entregó la versión final de la demanda a los Grupos 2 y 3 —no la estimación pre-intercambio de la Sección 3.3—. Con \texttt{resultado\_grupo6} (Sección 23), los outputs finales entregados son:
\begin{itemize}
    \item \textbf{Demanda médica para Grupo 2:} la demanda total a las 72h sube de 9,147 a \textbf{23,474 atenciones médicas} (+156\%). La mayor concentración se mantiene en Z5 (6,120) y ahora también en Z3 (5,684), que en la estimación inicial no destacaba.
    \item \textbf{Demanda de víveres para Grupo 3:} sube de 8,568 a \textbf{11,800 raciones por bloque de 6h} a nivel ciudad (capacidad operativa total de las 5 zonas), ya que ahora las 5 zonas —incluida Z4— llegan a su capacidad máxima en vez de solo 4.
    \item \textbf{Mapa de saturación de albergues:} se actualiza para reflejar que Z4 también supera el 80\% en el Bloque 0 (96.5\%), en vez de nunca saturarse.
\end{itemize}
Los tres archivos de \texttt{outputs/} entregados a Grupo 2, Grupo 3 y ambos (mapa de saturación) contienen esta versión final (Sección 24.2 del notebook); las cifras de la Sección 3.3 quedan documentadas únicamente como la estimación inicial, según lo exige el requisito de trazabilidad del intercambio.
...
El modelo asume que cada zona opera de forma completamente aislada ($\text{Reubicados}_z = 0$). En el escenario pre-intercambio, esto generaba una situación poco realista: mientras en Z1 y Z5 había más de 35,000 personas en la calle, en Z4 parecían existir más de 3,200 camas desocupadas que nadie utilizaba. Sin embargo, el output real del Grupo 6 (Sección 4) muestra que Z4 tampoco tiene margen real una vez que se considera la demanda de toda la ciudad, por lo que la reubicación interzonal ya no es una solución de holgura gratuita: cualquier estrategia de reubicación tendría que competir con instalaciones adicionales (carpas, escuelas, galpones), no solo con cupos ya disponibles.
